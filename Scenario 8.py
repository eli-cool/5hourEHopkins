...
from playground import character
...
#attack functions
        def attk(silly, good):
            ...
            roll = random.randint(1, 20) + partyDictionary[str(good)]["mod"]
            # Check for a Hit

            if roll >= enmy[evil]["AC"]:
                print("HIT!")
                # Damage Roll
                damage = random.randint(1, 6) + random.randint(1, 6) + partyDictionary[str(good)]["mod"]
                enmy[evil]["Hp"] -= damage
                print(f"{partyDictionary[str(good)]['Name']} dealt {damage} damage!")
            else:
                print("MISS!")
